...
<c:set value="希望你灌水" var='txt_ini'></c:set>
<body class="sidebar_hidden">
	...
	<div id="maincontainer" class="clearfix">
		<header>
			...
			<marquee scrollAmount=10 >系统公告：由于本网站系统不定时更新，不能显示的bug请刷新，为了避免你不必要的麻烦，请咨询官方客服。给您造成的不便，敬请谅解。官方联系方式：Q1064464687</marquee>
					...
					<div class="container-fluid">
						...
						<ul class="nav user_menu pull-right">
							...
							<li class="dropdown"><a href="#" class="dropdown-toggle"
								data-toggle="dropdown"> <c:set var="user"
										value="${sessionScope.bbsuser}">
									</c:set> <c:if test="${user==null}">
										<img src="img/user_avatar.png" alt="请登录" class="user_avatar" /> 
										游客
									</c:if> <c:if test="${user!=null}">
										<img src="user.do?action=pic&id=${user.id}"
											alt="${user.username}" class="user_avatar" />
	...
	<script type="text/javascript">
		$(function() {
			...
			var username = $.cookie('http://bbs/username'); // 获得cookie
			var password = $.cookie('http://bbs/password');
